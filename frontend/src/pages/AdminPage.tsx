...
const AdminPage: React.FC = () => {
  ...
  const applyModeChange = async (newMode: 'ai' | 'single_ai' | 'rule') => {
    ...
    if (newMode === 'ai') {
      ...
      if (!isConfigured) {
        message.warning('无法切换到多路综合判断模式：请先完成庄、闲、综合三个接口的配置。');
        return;
      }
    }
    if (newMode === 'single_ai') {
      ...
      if (!isConfigured) {
        message.warning('无法切换到智能判断模式：你还没有配置智能判断接口。');
        return;
      }
    }
  ...
  const loadSingleAiPromptTemplates = useCallback(async () => {
    ...
    try {
      ...
      message.error(err instanceof Error ? err.message : '加载智能判断说明模板失败');
    } finally {
  ...
  const saveSingleAiPromptTemplates = useCallback(async () => {
    ...
    try {
      ...
      message.success('智能判断说明模板已保存并生效');
      loadSingleAiPromptTemplates();
    } catch (err: any) {
      message.error(err instanceof Error ? err.message : '保存智能判断说明模板失败');
    } finally {
    ...
    <div className="page-wrapper admin-page">
      ...
      <Tabs
        ...
        items={[
          {
            key: 'ai',
            label: <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Icons.Robot /> {isMobile ? '模型设置' : '模型设置与判断方式'}</span>,
            children: (
                        ...
                        <div className="admin-mode-option" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                          <div style={{ display: 'grid', gap: 6 }}>
                            ...
                            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)' }}>
                              三个判断接口分别给出意见后再综合结论，适合更重视稳定性的使用方式。
                            </div>
                      ...
                      <Card size="small">
                        <div className="admin-mode-option" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                          <div style={{ display: 'grid', gap: 6 }}>
                            ...
                            <Space wrap className="mobile-action-row" style={{ width: '100%' }}>
                              <Button size="small" onClick={() => handleOpenApiConfig('single')}>设置智能判断接口</Button>
                            </Space>
                          </div>
                          <Button
                            className="admin-mode-option-action"
                  ...
                  {singleAiPromptLoading ? (
                    ...
                    <Space orientation="vertical" size={10} style={{ width: '100%' }}>
                      <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12 }}>
                        智能判断模式每一局都会自动分析、自动预测并自动下注。这里配置的是系统用于生成判断的说明文字，默认已经内置推荐版本。
                      </div>
                      ...
                      <Input.TextArea
                        ...
                        placeholder="请输入用于生成本局智能判断的中文说明。系统会自动补入靴号、局号、历史记录、路图特征和最近失误信息。"
                      ...
                      <Input.TextArea
                        ...
                        placeholder="请输入等待开奖阶段的智能判断说明。系统会自动补入历史、路图特征和连续失误次数。"
                        style={{ background: 'rgba(0,0,0,0.2)', borderColor: 'rgba(255,255,255,0.1)', color: '#fff' }}
                      />
                  ...
                  <Space style={{ marginBottom: 12, flexWrap: 'wrap' }} className="mobile-action-row">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        { label: MODE_LABELS.single_ai, value: 'single_ai' },
                      ]}
                      size="small"
                    />
                  </Space>
