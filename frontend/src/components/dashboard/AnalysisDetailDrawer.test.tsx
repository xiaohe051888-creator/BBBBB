...
describe('AnalysisDetailDrawer', () => {
  ...
  it('renders a prominent body close action and calls onClose when clicked', async () => {
    (globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
    const originalGetComputedStyle = window.getComputedStyle;
    window.getComputedStyle = ((element: Element) => {
      const style = originalGetComputedStyle(element);
    ...
    const bodyCloseButton = Array.from(document.querySelectorAll('button')).find((button) => button.textContent?.includes('我知道了，收起详情'));
    expect(bodyCloseButton).toBeTruthy();

    await act(async () => {
      bodyCloseButton?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
