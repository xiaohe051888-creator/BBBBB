# 状态一致性加固设计（JWT 固化 / DB 路径固定 / 默认规则模式 / 严格门禁）
...
### C）默认 rule + 严格门禁：后端模式门禁 + 启动纠偏 + 分析门禁
...
2. 启动纠偏：
   - 若 DB/内存中的 `prediction_mode` 是 `ai` 或 `single_ai`，但对应 key 未配置：
     - 自动设置为 `rule` 并写回 DB 与内存会话
     - 仅记录“已纠偏至 rule”提示，不输出任何密钥内容

3. 分析执行路径（`run_ai_analysis`）：
   - 若当前模式所需 key 缺失：
     - 直接阻止执行并返回明确错误（不输出其它引擎结果、不切换模式）
     - 确保不会卡死在“分析中”（保留安全回退结构）

4. 测试策略：
   - 新增：启动纠偏测试（无 key + DB mode=ai => 启动后变 rule）
   - 替换：原“缺 key 自动降级”的单元测试为“缺 key 阻止切换/阻止分析”的测试
