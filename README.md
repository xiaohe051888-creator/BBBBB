...
基于 React Web + FastAPI 打造的量化自动托管分析系统。本系统结合 **三模型协作分析** 与 **规则参考引擎**，实现对博弈趋势的快速解析、自动下注与资金风控。
...
- **综合模型（双子星平台）**：统筹决策，结合系统特有的**等待期微学习**与**复盘记录记忆**进行复盘进化。
...
- **隔离污染**：如果是“规则参考引擎”在运行，系统会从物理层面切断大模型的令牌消耗，绝不触发微学习和复盘记录联动，做到双脑 100% 物理隔离。
...
迁移脚本会把核心业务表（状态、记录、下注、日志、复盘记录、五路缓存、记忆库、版本表）迁移到 Postgres。迁移完成后，生产环境只需要设置 `DATABASE_URL` 即可。
